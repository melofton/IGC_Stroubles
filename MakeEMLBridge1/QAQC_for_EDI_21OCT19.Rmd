...
B1 <- left_join(B1_WQ_merge, B1_stage_merge) %>%
  mutate(Stage_interp = na.approx(Lvl_m, maxgap = 4, rule = 2),
         Flag_SpCond_uScm = ifelse(!is.na(Stage_interp) & Stage_interp<0.25,ifelse(Flag_SpCond_uScm == 0,2,12), Flag_SpCond_uScm),
  ...
  select(-Lvl_m,-Discharge_cms,-Flag_Lvl_m,-Flag_Discharge_cms,-Stage_interp)
