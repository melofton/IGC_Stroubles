...
B1_stage_QA_2 <- B1_stage_QA_1 %>%
  mutate(Lvl_m = na.approx(Lvl_m, maxgap = 12, rule = 1), #interpolating between NAs for water level
         ...
         Site = ifelse(is.na(Site),"Bridge_1",Site)) #filling NAs in Site column with "Bridge 1"
...
B1_stage_QA <- B1_stage_QA_2 %>% #adding flags if Lvl_m is above 4 (!!)
  mutate(Flag_Lvl_m = ifelse(Lvl_m > 4, 1, 0),
         ...
         Discharge_cms_perc = ifelse(Lvl_m >= quantile(Discharge_cms, probs = 0.90, na.rm = TRUE),1,0)) #adding flag for 90th percentile of Discharge
...
B1 <- left_join(B1_WQ_merge, B1_stage_merge) %>%
  mutate(Flag_SpCond_uScm = ifelse(!is.na(Lvl_m) & Lvl_m<0.26, ifelse(Flag_SpCond_uScm == 0,2,12), Flag_SpCond_uScm),
         Flag_pH = ifelse(!is.na(Lvl_m) & Lvl_m < 0.2, ifelse(Flag_pH == 0,2,12), Flag_pH),
         Flag_Turb_NTU = ifelse(!is.na(Lvl_m) & Lvl_m < 0.16 , ifelse(Flag_Turb_NTU == 0,2,12), Flag_Turb_NTU)) 
  
write.csv(B1, "c:/Users/Mary Lofton/Documents/RProjects/IGC_Stroubles/MakeEMLBridge1/Bridge_1.csv",row.names = FALSE)
