...
```{r select only storms where it seems like sensor wasn't out of the water at the start or end}
badstorms <- c()
...
filtered_storms_info <- filter( storms_info, !(Storm_ID %in% badstorms))
...
ggplot( filtered_storms_info, mapping = aes( x = parse_date_time(Storm_ID, "y-m-d H:M:S"), y = timelag_min, color = Season)) +
  geom_point() +
...
for( ID in filtered_storms_info$Storm_ID ){
  
  graphstart <- Sys.time()
  
  #pick out the data we're going to use 
  stormdata <- filter(B1, Storm_ID == ID)
  stormmetadata <- filter( filtered_storms_info, Storm_ID == ID)
