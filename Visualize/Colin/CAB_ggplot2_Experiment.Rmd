...
MaxDischarge.Plot <- ggplot( Month_MaxMin_Dis, aes( x = Year + Month/12, y = MaxDischarge, color = Month) ) + geom_point() + geom_path() + labs( title = "Maximum Discharge by Month") +
  annotate( "point", x = min(B1_QA$Year + B1_QA$Month/12 - .5), y = quantile(B1_QA$Discharge, 0.9, names = FALSE), ) +
  annotate( "text", x = min(B1_QA$Year + B1_QA$Month/12 - .5), y = quantile(B1_QA$Discharge, 0.9, names = FALSE), label = "90th Quantile")
