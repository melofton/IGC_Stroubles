...
MaxDischarge.Plot <- ggplot( Month_MaxMin_Dis, aes( x = Year + Month/12, y = MaxDischarge, color = Month) ) + geom_point() + geom_path() + labs( title = "Maximum Discharge by Month") +
  ...
  annotate( "text", x = min(B1$Year + B1$Month/12 - .5), y = quantile(B1$Discharge, 0.9, names = FALSE), label = "90th Quantile")
...
June02.13 <- subset(B1, DateTime > "2013-03-31 0:00:00" & DateTime < "2013-04-01 23:45:00" )

ggplot(June02.13, aes(x=Discharge, y=SpCond_uScm) ) +
...
```{r graph every discharge point}

#Look at patterns of discharge to identify storms and rainy seasons
graph1 <- ggplot(data = B1, aes(x = DateTime, y = Discharge, color = Flag_NinetyQDischarge)) + geom_point() + geom_path()
