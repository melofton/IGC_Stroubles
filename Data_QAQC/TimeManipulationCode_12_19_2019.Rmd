...
```{r}
setwd("C:/Users/laure/Google Drive/Research/PHD/IGC Mini Project/Preliminary Data")
...
```{r}
plot2 <- ggplot(data = B1, aes(x = Discharge, y = Conductivity_Sp, colour = Flag_SpCond_uScm))+
  geom_point()+
  theme_bw()
plot2
```
...
```{r}
B1_1 <- B1 %>% mutate(Date = as.POSIXct(DateTime,format="%m/%d/%Y %H:%M",tz=Sys.timezone()),
         Time = X2,
...
```{r}
plot3data <- subset(B1,month(B1$DateTime) == October %in% c(9:10))
plot3 <- ggplot(data = plot3data, aes(x = DateTime, y = Conductivity_Sp, colour = Flag_SpCond_uScm))+
  geom_point()+
  theme_bw()
plot3
```
