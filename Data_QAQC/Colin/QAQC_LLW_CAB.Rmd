...
B1_stage_2 <- add_row( filter( B1_stage, minute(DateTime) != 10 & minute(DateTime) != 20 & minute(DateTime) != 40 & minute(DateTime) != 50 ),
  ...
  arrange(DateTime)
#this creates stage on the 0 and 30s, not merged
...
#this is just 0 and 30s, 15 and 45 not there...   
...
#Rename "B1_QA" "B1." This the name every other "colin file" assumes the water quality dataset is called. Also, remove all the other objects QAQCing has created.
```{r}
B1 <- B1_QA

rm(B1_QA, B1_intrp_stage, B1_stage_QA, B1_stage, B1_WQ)
```
