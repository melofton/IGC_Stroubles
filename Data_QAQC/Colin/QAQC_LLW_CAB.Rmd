...
#Bridge 1 WQ, stage, and discharge merged into one sheet. Overall QAQC for Bridge 1
```{r}
...
#Flag values if they're too high or low, or if it seems like the Stroubles wasn't high enough to cover the sensor on the sonde.
B1_QA <- B1_WQ %>%
  mutate(Flag_Temp_degC = ifelse(Temp_degC < -5 | Temp_degC > 30, 1, 0),
         Flag_SpCond_uScm = ifelse(SpCond_uScm < 50 | SpCond_uScm > 1500 | Lvl_m < 0.25 , 1, 0),
         Flag_pH = ifelse(pH <6 | pH >9 | Lvl_m <0.2, 1, 0),
         Flag_Turb_NTU = ifelse(Turb_NTU < 0 | Turb_NTU > 1000 | Lvl_m < 0.16 , 1, 0), #Timpano et al. 2010
