...
#Plotting DO (mgL)
```{r}
#time series with color highlighting flagged points
plotdata <- subset(B1_QA,year(B1_QA$DateTime) == 2018)
plotdata6 <- subset(B1_QA,year(B1_QA$DateTime) == 2014)

plot1 <- ggplot(data = B1_QA, aes(x = DateTime, y = DO_mgL, colour = as.factor(Flag_DO_mgL))) +
  geom_point() +
  theme_bw()
plot1

plot2 <- ggplot(data = plotdata, aes(x = DateTime, y = DO_mgL, colour = as.factor(Flag_DO_mgL))) +
  geom_point() +
  theme_bw()
plot2

plot3data <- subset(B1_QA, Flag_DO_mgL == 0 & year(B1_QA$DateTime) %in% c(2015:2017) & month(B1_QA$DateTime) %in% c(2:5))

plot3 <- ggplot(data = plot3data, aes(x = DateTime, y = DO_mgL)) +
  geom_point() +
  theme_bw()
plot3

plot4data <- B1_QA %>% 
  filter(Flag_DO_mgL == 0 & year(DateTime) == 2017) %>%
  mutate(Day = date(DateTime)) %>%
  group_by(Day) %>%
  summarize(daily_DO_avg = mean(DO_mgL))

plot4 <- ggplot(data = plot4data, aes(x = Day, y = daily_DO_avg)) +
  geom_point() +
  theme_bw()
plot4

plot5 <- ggplot(data = B1_QA, aes(x = DO_mgL, y = SpCond_uScm)) +
  geom_point() +
  theme_bw()
plot5

plot6 <- ggplot(data = plotdata6, aes(x = DateTime, y = DO_mgL, colour = as.factor(Flag_DO_mgL))) +
  geom_point() +
  theme_bw()
plot6
```
