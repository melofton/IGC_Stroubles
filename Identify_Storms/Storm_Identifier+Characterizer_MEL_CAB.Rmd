...
#This version identifies a storm both when discharge drops below the 90th percentile and when there's over an hour of difference between the DateTime of the current and previous "storm" rows. To do this, the loop works with the full, finalized dataset instead of only the portion including storms. 
...
#This needs to use the full B1 data set, so it happens before storms_info is created. The volume calculations are added to each row within their respective storm.
...
#summary info includes volume, time length, magnitude (volume / timelength)
