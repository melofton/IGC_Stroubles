...
#The finalized sheet used by this document is "B1," not B1_QA!
...
#Needs:
#a list of storm objects. Set up the storm "placeholder_date"
storms <- list(NULL)

placeholder_date <- as.POSIXct(0, origin = "1970-01-01", tz = Sys.timezone())
storm <- list( start = placeholder_date )
...
last_stormr_datetime <- placeholder_date
...
while( n_current_row <= nrow(B1) ){
  ...
  if( B1$Flag_High_Discharge[n_current_row] == 1 ){
    #If the last "storm" row was more than an hour ago, create a new storm object. Otherwise, add to the current one.
    if( ( B1$DateTime[n_current_row] %>% as.numeric() - last_stormr_datetime %>% as.numeric() ) >= 3600 ){
      ...
      if( last_stormr_datetime != placeholder_date ){
        storms <- list( unlist(storms), assign( paste(storm$start), unlist(storm) ) ) 
      }
      #create a new storm object. Inside it, record the first WQ row and the storm's start time
      storm <- list( tbl = B1[ n_current_row, ], start = B1$DateTime[n_current_row] )
    } else{
      storm$tbl <- storm$tbl %>% bind_rows( B1[n_current_row, ])
    }
  ...
  n_current_row <- n_current_row + 1
}

rm( last_stormr_datetime, placeholder_date, n_current_row, storm )
```
