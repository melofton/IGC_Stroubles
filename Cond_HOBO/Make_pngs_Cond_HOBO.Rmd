...
##HISTOGRAM OF LVL_M
hist(B1$Lvl_m,  breaks = seq(0,2,by = 0.01),main = "Red line is if we cut off at 0.1 m")
...
#Pngs with a Lvl_m cutoff at 0.1 m
```{r}
...
B1_viz <- B1 %>%
  mutate(Discharge_cms_perc = ifelse(Discharge >= quantile(Discharge, probs = 0.90, na.rm = TRUE),1,0),
         Flag_SpCond_uScm = ifelse(!is.na(Lvl_m) & Lvl_m < 0.1,1,Flag_SpCond_uScm)) %>%
